...
//	m_MainSplitter.CreateView(0, 0, RUNTIME_CLASS(CMyForm), CSize(300, TOOL_WINCY), pContext);
	...
	const _uint second_WinX = 350;

	m_SecondSplitter.CreateView(0, 0, RUNTIME_CLASS(CMyForm), CSize(second_WinX, 500), pContext);
	m_SecondSplitter.CreateView(1, 0, RUNTIME_CLASS(CMiniView), CSize(second_WinX, 80), pContext);

	// 열 번호, 열의 크기 지정, 허용 가능한 최소 크기
	m_MainSplitter.SetColumnInfo(0, second_WinX, 20);
