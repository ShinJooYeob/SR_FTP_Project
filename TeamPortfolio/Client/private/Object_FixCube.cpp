...
_int CObject_FixCube::Obsever_On_Trigger(CGameObject* pDestObjects, _float3 fCollision_Distance, _float fDeltaTime)
{
	...
	{
		if (GetSingle(CGameInstance)->Get_DIKeyState(DIK_LSHIFT) & DIS_Up)
		{
			Particle_Dust();
		}
	}
...
void CObject_FixCube::Particle_Dust()
{
	PARTICLEDESC tDesc;
	//파티클이 흩날리는 종류 설정
	...
	tDesc.FollowingTarget = m_ComTransform;//(CTransform*)(GetSingle(CGameInstance)->Get_GameObject_By_LayerIndex(SCENE_STATIC, TAG_LAY(Layer_Player))->Get_Component(TAG_COM(Com_Transform)));
