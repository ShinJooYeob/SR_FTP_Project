...
HRESULT CScene_IMGUI::Ready_Layer_Terrain(list<SPECIALCUBE*>* listdata)
{
	...
	for (auto data: *listdata)
	{
		...
		if (data->Tagname == TAG_OP(Prototype_PortalCube_A))
		{
			if (count_Potal % 2 == 0)
			{
				memcpy(potalDesc.vPos_A_Cube, &(data->WorldMat[3]), sizeof(_float3));

			}
			else
			{
				memcpy(potalDesc.vPos_B_Cube,&(data->WorldMat[3]),sizeof(_float3));
				// 생성
				GetSingle(CGameInstance)->Add_GameObject_To_Layer(potalDesc.iNowScene,
